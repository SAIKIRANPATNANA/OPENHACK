...
def create_blood_test_plots(report_data, output_folder='static/plots'):
    try:
        for param, details in report_data.get('lab_results', {}).items():
            if details['status'] != 'Normal':
                try:
                    # Clean the value by removing status indicators
                    value = details['value']
                    if isinstance(value, str):
                        # Remove 'L' or 'H' and convert to float
                    ...
                    # Plot the value
                    plt.bar(['Current Value'], [value], color='blue', alpha=0.6)
                    
                    # Plot reference range if available
                    if 'reference_range' in details:
                        ref_range = details['reference_range']
                        if '-' in ref_range:
                            min_val, max_val = map(float, ref_range.split('-'))
                            plt.axhline(y=min_val, color='r', linestyle='--', label='Min Reference')
                            plt.axhline(y=max_val, color='r', linestyle='--', label='Max Reference')
                    
                    plt.title(f'{param} Level')
                    if 'unit' in details:
                        plt.ylabel(f'Value ({details["unit"]})')
                    plt.legend()
                    
                    # Save the plot
                    safe_param = param.replace(' ', '_').replace(',', '').lower()
                    plt.savefig(os.path.join(output_folder, f'{safe_param}.png'))
                    plt.close()
                
                except Exception as e:
                    print(f"Error creating plot for {param}: {str(e)}")
                    continue
    
    except Exception as e:
        print(f"Error in create_blood_test_plots: {str(e)}")
        raise ParseError(f"Failed to create plots: {str(e)}")
